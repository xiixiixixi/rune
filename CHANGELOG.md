...
- 修复从项目调试副本检查更新后，“重试”按钮只会再次失败、无法回到正式安装版的问题。
- 更新器会验证并识别系统或用户“应用程序”文件夹中的 Rune，把主要操作改为“打开正式版”。
- 切换时强制启动正式安装版的新进程，避免系统因包标识相同又激活调试副本；正式版启动后会立即继续检查更新。
- 如果没有安装正式版，则明确提供“手动下载”，不再让用户进入无效重试循环。
- 继续禁止自动更新覆盖项目构建目录，保留原有安全边界。

### 验证

- 在真实更新窗口中完成“项目调试副本 → 应用程序中的 0.7.3 → 自动更新至 0.7.4”的完整流程。
- 确认更新后进程来自 `/Applications/Rune.app`、应用签名有效，且没有遗留 `Rune.app.old` 备份。
- CaptureKit 26 项自动测试、调试构建和 Universal 2 双架构正式构建通过。
